    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  // Transformation notes from vehicles coordinates:
		  // Note: px = 0, py = 0 and psi = 0 because this is the center of car space
		  // xm = (cos(psi) * (ptsx[i] - px)) - (sin(psi) * (ptsy[i] - py))
